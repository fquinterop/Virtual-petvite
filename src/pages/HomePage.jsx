...
// Colores exactos del logo VirtualPet
const TEAL = '#3D8A8C'
const ORANGE = '#D4883B'
...
const CATEGORIES = [
  ...
  { emoji: '🦴', name: 'Snacks y premios',     to: '/products?cat=snacks' },
  ...
  return (
    <div style={{ fontFamily: "'Segoe UI', sans-serif", color: '#1a1a1a', background: '#fff' }}>
      ...
      <div style={{
        ...
        fontSize: '0.78rem', padding: '0.4rem 2rem',
        ...
        <span style={{ opacity: 0.8 }}>Síguenos en redes</span>
      </div>
      ...
      <section style={{
        ...
        <div style={{ maxWidth: 960, margin: '0 auto', display: 'flex', justifyContent: 'space-between', alignItems: 'center', gap: '2rem', flexWrap: 'wrap' }}>
          ...
          <div style={{ flex: 1, minWidth: 280 }}>
            ...
            <h1 style={{
              color: '#fff', fontSize: 'clamp(1.6rem, 4vw, 2.5rem)',
              fontWeight: 800, lineHeight: 1.25, margin: '0 0 0.75rem',
            }}>
          ...
          <div style={{ display: 'flex', gap: '2.5rem', flexShrink: 0 }}>
            {[['133+','Productos'],['10','Categorías'],['15+','Marcas']].map(([n,l]) => (
              <div key={l} style={{ textAlign: 'center', color: '#fff' }}>
                ...
                <p style={{ fontSize: '0.75rem', opacity: 0.75, margin: 0, marginTop: '0.2rem' }}>{l}</p>
              </div>
            ))}
          </div>
        </div>
      ...
      <section style={{ maxWidth: 960, margin: '2rem auto', padding: '0 1.5rem', display: 'grid', gridTemplateColumns: 'repeat(auto-fit,minmax(300px,1fr))', gap: '1.25rem' }}>
        {/* Perros */}
        <div style={{
          background: LIGHT_ORANGE, borderRadius: 16, padding: '2rem',
          display: 'flex', justifyContent: 'space-between', alignItems: 'center',
        }}>
          <div>
            <h3 style={{ margin: '0 0 0.4rem', fontWeight: 800, color: '#1a1a1a' }}>Cuida a tu mejor amigo 🐶</h3>
        ...
        {/* Gatos */}
        <div style={{
          background: LIGHT_TEAL, borderRadius: 16, padding: '2rem',
          display: 'flex', justifyContent: 'space-between', alignItems: 'center',
        }}>
          <div>
            <h3 style={{ margin: '0 0 0.4rem', fontWeight: 800, color: '#1a1a1a' }}>Mima a tu felino 🐱</h3>
            <p style={{ color: '#555', fontSize: '0.85rem', margin: '0 0 1rem', lineHeight: 1.5 }}>
      ...
      <section style={{ maxWidth: 960, margin: '0 auto 2.5rem', padding: '0 1.5rem' }}>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(190px,1fr))', gap: '1rem' }}>
          {[
            { badge: 'Super Premium', label: 'Alimento para perros', name: 'Alimento Premium PRD001', price: 42900, emoji: '🐶', bg: LIGHT_TEAL },
            { badge: 'Grain Free',    label: 'Alimento para gatos',  name: 'Alimento Premium PRD003', price: 38900, emoji: '🐱', bg: LIGHT_ORANGE },
            { badge: 'Más vendido',   label: 'Savic',                name: 'Comedero Cena Antideslizante', price: 28900, emoji: '🥣', bg: LIGHT_TEAL },
            { badge: 'Natural',       label: 'Dental Bone',          name: 'Dental Bone 3" x 5 uds',  price: 22900, emoji: '🦴', bg: LIGHT_ORANGE },
          ].map((p) => (
            <div key={p.name} style={{
              background: '#fff', borderRadius: 12, overflow: 'hidden',
              boxShadow: '0 1px 8px rgba(0,0,0,0.08)', border: '1px solid #f0f0f0',
            }}>
              <div style={{
                background: p.bg, height: 130,
              ...
              <div style={{ padding: '0.75rem' }}>
                ...
                <p style={{ fontWeight: 800, color: DARK_TEAL, margin: '0 0 0.75rem', fontSize: '0.95rem' }}>
                  ${p.price.toLocaleString()}
                </p>
                <Link to="/products">
                  <button style={{
                    width: '100%', background: ORANGE, color: '#fff', border: 'none',
                    borderRadius: 8, padding: '0.5rem', fontWeight: 700,
